...
- **RPC 비밀번호**: firpeng (기본값, 환경 변수 `BITCOIN_RPC_PASSWORD`로 변경 가능)
...
1. **RPC 비밀번호 변경**: 기본 비밀번호(`firpeng`)를 반드시 변경하세요
